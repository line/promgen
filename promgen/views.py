...
class HostList(LoginRequiredMixin, ListView):
    queryset = models.Host.objects.prefetch_related(
        "farm", "farm__project_set", "farm__project_set__service",
    )
...
class NotifierUpdate(LoginRequiredMixin, UpdateView):
    ...
    def post(self, request, pk):
        ...
        if "filter.name" in request.POST:
            ...
            if created:
                messages.success(
                    request, "Created filter {f.name} {f.value}".format(f=f)
                )
            else:
                messages.warning(
                    request, "Updated filter {f.name} {f.value}".format(f=f)
                )
...
class NotifierTest(LoginRequiredMixin, View):
    def post(self, request, pk):
        ...
        try:
            ...
            messages.warning(
                request, "Error sending test message with " + sender.sender
            )
        else:
...
class FarmList(LoginRequiredMixin, ListView):
    paginate_by = 50
    queryset = models.Farm.objects.prefetch_related("project_set", "host_set",)
...
class FarmUpdate(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        farm, created = models.Farm.objects.update_or_create(
            id=self.kwargs["pk"], defaults=form.clean(),
        )
        return HttpResponseRedirect(
            reverse("project-detail", args=[farm.project_set.first().id])
        )
...
class FarmLink(LoginRequiredMixin, View):
    ...
    def post(self, request, pk, source):
        ...
        farm, created = models.Farm.objects.get_or_create(
            name=request.POST["farm"], source=source,
        )
        if created:
...
class URLList(LoginRequiredMixin, ListView):
    queryset = models.URL.objects.prefetch_related(
        "project", "project__service", "project__shard", "probe",
    )
...
class RuleUpdate(mixins.PromgenPermissionMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.setdefault(
            "formset_annotations", forms.AnnotationFormset(instance=self.object)
        )
...
class ProjectNotifierRegister(LoginRequiredMixin, FormView, mixins.ProjectMixin):
    ...
    def form_valid(self, form):
        ...
        sender, created = models.Sender.objects.get_or_create(
            obj=project, owner=self.request.user, **form.clean()
        )
...
class ServiceNotifierRegister(LoginRequiredMixin, FormView, mixins.ServiceMixin):
    ...
    def form_valid(self, form):
        ...
        sender, created = models.Sender.objects.get_or_create(
            obj=service, owner=self.request.user, **form.clean()
        )
        signals.check_user_subscription(models.Sender, sender, created, self.request)
...
class Profile(LoginRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["notifiers"] = {
            "notifiers": models.Sender.objects.filter(obj=self.request.user)
        }
...
class RuleTest(LoginRequiredMixin, View):
    def post(self, request, pk):
        ...
        if metrics:
            ...
            context["errors"][
                "no_results"
            ] = "No Results. May need to remove conditional check (> < ==) to verity"
        ...
        return JsonResponse(
            {
                request.POST["target"]: render_to_string(
                    "promgen/ajax_clause_check.html", context
                )
            }
        )
