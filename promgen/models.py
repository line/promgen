...
class Service(models.Model):
    ...
    def default(cls, service_name="Default", shard_name="Default"):
        ...
        service, created = cls.objects.get_or_create(
            name=service_name, defaults={"shard": shard}
        )
        if created:
...
class Rule(models.Model):
    ...
    name = models.CharField(
        max_length=128, unique=True, validators=[validators.alphanumeric]
    )
    ...
    def copy_to(self, content_type, object_id):
        ...
        with transaction.atomic():
            ...
            self.name = "{}_{}".format(self.name, slugify(content_object.name)).replace(
                "-", "_"
            )
