...
class WebTests(tests.PromgenTest):
    ...
    def test_merge_users(self):
        ...
        self.assertTrue(
            social_auth.user == user_to_merge_into, "Social auth transferred to User_to_merge_into"
        )
        self.assertTrue(
            service.owner == user_to_merge_into, "Service owner changed to User_to_merge_into"
        )
        self.assertTrue(
            project.owner == user_to_merge_into, "Project owner changed to User_to_merge_into"
        )
        self.assertTrue(
            sender_email.owner == user_to_merge_into, "Sender owner changed to User_to_merge_into"
        )
        self.assertTrue(
            sender_user.owner == user_to_merge_into, "Sender owner changed to User_to_merge_into"
        )
        self.assertTrue(
            sender_user.value == str(user_to_merge_into.pk),
            "Sender value changed to User_to_merge_into",
        )
        self.assertEqual(
        ...
        self.assertTrue(
            ...
            "User_to_merge_into inherit existing permissions",
        )
        self.assertTrue(
            user_to_merge_into.groups.filter(pk=group.pk).exists(),
            "User_to_merge_into inherit group membership",
        )
        self.assertEqual(
            UserObjectPermission.objects.filter(
                user=user_to_merge_into,
        ...
        self.assertTrue(
            ...
            "User_to_merge_into inherit the highest permission on group",
        )
        self.assertFalse(
            UserObjectPermission.objects.filter(user_id=user_to_merge_from.pk).exists(),
            "User_to_merge_from's object permissions deleted",
        )
        self.assertFalse(
            User.objects.filter(pk=user_to_merge_from.pk).exists(), "User_to_merge_from was deleted"
        )
