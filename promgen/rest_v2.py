...
class FarmViewSet(
    ...
    def sources(self, request):
        ...
        if remote is not None:
            if remote.lower() == "true":
                ...
                raise ValidationError({"remote": "Invalid value for remote. Use true or false."})
    ...
    def register_host(self, request, id):
        ...
        return Response(
            serializers.HostListSerializer(
                {"hosts": sorted(valid_hosts, key=lambda host: host.lower())}
            ).data,
            status=HTTPStatus.CREATED,
        )
