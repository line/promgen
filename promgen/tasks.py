...
class BaseTaskWithRetry(Task):
    ...
    enabled to enhance reliability. To handle retried tasks in the DLQ, you can set up a separate
    worker that listens to the "promgen_dlq" queue by using the following command:
    ...
    retry_backoff = 5
    retry_jitter = False
    if getattr(settings, "CELERY_ENABLE_PROMGEN_DEAD_LETTER_QUEUE", True):
