    ...
    def __call__(self, request):
        ...
        if settings.ENABLE_API_LOGGING and request.path.startswith("/rest/v2/"):
            try:
                ...
                logger.info(
                    ...
                    f"body size: {len(request.body) if request.body else 0} bytes"
                )
                if request.body and request.headers["Content-Type"] == "application/json":
                    ...
                    truncated_body = self.truncate_json_fields(
                        request.body, self.ACCESS_LOG_FIELD_LIMITS
                    )
                    logger.info(f"[Trace ID: {request.trace_id}] Request body: {truncated_body}")
            except Exception as e:
                logger.exception(
                    f"[Trace ID: {request.trace_id}] "
                    f"An error occurred when parsing request: {str(e)}"
                )
        ...
        if settings.ENABLE_API_LOGGING and request.path.startswith("/rest/v2/"):
            try:
                ...
                logger.exception(
                    f"[Trace ID: {request.trace_id}] "
                    f"An error occurred when logging response: {str(e)}"
                )
