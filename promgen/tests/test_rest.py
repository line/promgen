...
class RestAPITest(tests.PromgenTest):
    ...
    def test_rest_user(self):
        ...
        response = self.client.get(reverse("api-v2:user-list"), {"page_number": 1, "page_size": 1})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)

        # Check retrieving users whose "username" contains "demo"
        expected = tests.Data("examples", "rest.user.filter_by_username.json").json()
        response = self.client.get(reverse("api-v2:user-list"), {"username": "demo"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)

        # Check retrieving users with a non-existent "username" returns an empty list
        response = self.client.get(reverse("api-v2:user-list"), {"username": "non-existent"})
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(reverse("api-v2:user-list"), {"email": "demo@example"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)

        # Check retrieving users with a non-existent "email" returns an empty list
        response = self.client.get(reverse("api-v2:user-list"), {"email": "non-existent"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["count"], 0)
    ...
    def test_rest_notifier(self):
        ...
        response = self.client.get(
            reverse("api-v2:sender-list"), {"page_number": 1, "page_size": 1}
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)
        ...
        response = self.client.get(reverse("api-v2:sender-list"), {"content_type": "service"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)

        # Check retrieving notifiers with a non-allowed "content_type" returns 400 Bad Request
        response = self.client.get(reverse("api-v2:sender-list"), {"content_type": "non-allowed"})
        self.assertEqual(response.status_code, 400)

        # Check retrieving notifiers whose "object_id" is "1"
        expected = tests.Data("examples", "rest.notifier.filter_by_object_id.json").json()
        response = self.client.get(reverse("api-v2:sender-list"), {"object_id": "1"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)
        ...
        response = self.client.get(reverse("api-v2:sender-list"), {"owner": "demo"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)

        # Check retrieving notifiers with a non-existent "owner" returns an empty list
        response = self.client.get(reverse("api-v2:sender-list"), {"owner": "non-existent"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["count"], 0)
        ...
        response = self.client.get(
            reverse("api-v2:sender-list"), {"sender": "promgen.notification.email"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)

        # Check retrieving notifiers with a non-existent "sender" returns 400 Bad Request
        response = self.client.get(reverse("api-v2:sender-list"), {"sender": "non-existent"})
        self.assertEqual(response.status_code, 400)
        ...
        response = self.client.get(reverse("api-v2:sender-list"), {"value": "services"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)

        # Check retrieving notifiers with a non-existent "value" returns an empty list
        response = self.client.get(reverse("api-v2:sender-list"), {"value": "non-existent"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["count"], 0)
        ...
        response = self.client.post(
            reverse("api-v2:sender-delete-filter", args=[1]),
            {"name": "name", "value": "value"},
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 401)
        ...
        response = self.client.post(
            reverse("api-v2:sender-delete-filter", args=[2]),
            {"name": "name", "value": "value"},
            content_type="application/json",
            HTTP_AUTHORIZATION=f"Token {token}",
        )
