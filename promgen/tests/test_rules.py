...
class RuleTest(PromgenTest):
    ...
    def test_copy(self, mock_post):
        ...
        self.assertCount(
            models.RuleLabel, 3, "Copied rule has exiting labels + service label"
        )
    ...
    def test_macro(self, mock_post):
        ...
        for k, r in rules.items():
            self.assertEquals(
                macro.rulemacro(r["model"]), r["assert"], "Expansion wrong for %s" % k
            )
    ...
    def test_invalid_annotation(self, mock_post):
        ...
        models.RuleAnnotation.objects.create(
            name="summary", value="{{$label.foo}}", rule=self.rule
        )
